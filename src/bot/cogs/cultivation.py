...
class CultivationCog(commands.Cog, name="Cultivation"):
    ...
    async def status(self, interaction: discord.Interaction) -> None:
        ...
        async with get_session() as session:
            ...
            char = _orm_to_model(player)
            embed = base_embed(title=f"📜 Trạng Thái: {char.name}")
            
            # Hiển thị 3 trục tu luyện
            for ax in ["qi", "body", "formation"]:
                xp = getattr(char, f"{ax}_xp")
                r_idx = getattr(char, f"{ax}_realm")
                label = realm_label(ax, r_idx, xp)
                embed.add_field(name=f"{AXIS_ICONS[ax]} {AXIS_LABELS[ax]}", value=f"**{label}**\nEXP: `{xp:,}`", inline=True)
            
            embed.add_field(name="✨ Công Đức", value=f"`{char.merit:,}`", inline=False)
