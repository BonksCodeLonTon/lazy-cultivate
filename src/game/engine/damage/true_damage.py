...
-------
    ...
    bonus    = max(1, int(base_damage * pct))
    if is_crit: bonus *= 1.5
    target.take_damage(bonus)

Note that ``base_damage`` is the *post-mitigation* damage the attacker
just landed on the target — the true-damage scalar is applied on top of
that, so a stronger hit naturally produces a stronger true-damage chip.
This replaces the legacy ``target.hp_max × pct`` model where a low-ATK
build could chip a fixed % of any target regardless of the actual hit's
strength. The new model:
  ...
  * Keeps the ``crit × 1.5`` multiplier so crit-true-damage builds stay
    meaningful.
"""
