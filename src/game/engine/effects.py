...
# ── Linh Căn combat procs ─────────────────────────────────────────────────────
# All active Linh Căn proc logic lives here so display messages use
# description_vi from the EFFECTS registry (data-driven, no hardcoded strings).

def _apply_linh_can_effect(
    actor: "Combatant",
    target: "Combatant",
    effect_key: str,
    duration: int,
    ...
    """Apply a linh_can-triggered effect to target, logging with description_vi."""
    meta = EFFECTS.get(effect_key)
    if not meta:
        return
    target.apply_effect(effect_key, duration)
    log.append(
        f"    {meta.emoji} **{actor.name}** {trigger_label} — "
        f"**{meta.vi}** ({duration}t): *{meta.description_vi}*"
    )


def try_linh_can_dodge(
    target: "Combatant", rng: random.Random, log: list[str]
) -> bool:
    """[Phong] Tốc Biến — 10% chance to fully dodge the incoming attack."""
    if "phong" not in target.linh_can or rng.random() >= 0.10:
        return False
    log.append(f"  🌪️ **{target.name}** [Phong] Tốc Biến — né tránh hoàn toàn!")
    return True


def try_linh_can_cleanse(
    actor: "Combatant", rng: random.Random, log: list[str]
) -> None:
    """[Quang] Thanh Tẩy — 15% chance to cleanse 1 debuff/CC and restore 5% MP."""
    if "quang" not in actor.linh_can or rng.random() >= 0.15:
        return
    ...
    removed_name = removed_meta.vi if removed_meta else removed
    log.append(
        f"  ✨ **{actor.name}** [Quang] Thanh Tẩy — giải **{removed_name}** | +{mp_restore} MP"
    )


def get_linh_can_pen_pct(
    actor: "Combatant", rng: random.Random, log: list[str]
) -> float:
    """[Kim] Xuyên Thấu — 20% chance to ignore 30% of elemental resistance this hit."""
    if "kim" not in actor.linh_can or rng.random() >= 0.20:
        return 0.0
    log.append(f"  ⚙️ **{actor.name}** [Kim] Xuyên Thấu kích hoạt — xuyên 30% kháng nguyên tố!")
    return 0.30


def apply_linh_can_on_hit(
    actor: "Combatant",
    target: "Combatant",
    dmg: int,
    is_crit: bool,
    ...
    """Apply all on-hit Linh Căn procs (Hỏa/Thủy/Lôi/Ám) for actor's spiritual roots."""
    if dmg <= 0:
        return
    # [Hỏa] Bạo Liệt — 20% chance to burn target
    if "hoa" in actor.linh_can and rng.random() < 0.20:
        _apply_linh_can_effect(actor, target, EffectKey.DEBUFF_THIEU_DOT, 2, "[Hỏa] Bạo Liệt", log)
    # [Thủy] Ngưng Đọng — 15% chance to slow target
    if "thuy" in actor.linh_can and rng.random() < 0.15:
        _apply_linh_can_effect(actor, target, EffectKey.EFFECT_NGUNG_DONG, 2, "[Thủy] Ngưng Đọng", log)
    # [Lôi] Tê Liệt — 12% chance on critical hit to stun target
    if "loi" in actor.linh_can and is_crit and rng.random() < 0.12:
        _apply_linh_can_effect(actor, target, EffectKey.CC_STUN, 1, "[Lôi] Tê Liệt", log)
    # [Ám] Hủ Thể — 15% chance to lifesteal 30% of damage dealt
    if "am" in actor.linh_can and rng.random() < 0.15:
        steal = int(dmg * 0.30)
        actor.hp = min(actor.hp_max, actor.hp + steal)
        log.append(f"    🌑 **{actor.name}** [Ám] Hủ Thể — hút {steal:,} HP!")


def check_linh_can_shield(combatant: "Combatant", log: list[str]) -> None:
    """[Thổ] Hộ Thể — once per combat: when HP ≤ 35%, gain shield for 20% max HP."""
    if "tho" not in combatant.linh_can or combatant.ho_the_used or not combatant.is_alive():
        return
    if combatant.hp <= int(combatant.hp_max * 0.35):
        shield = int(combatant.hp_max * 0.20)
