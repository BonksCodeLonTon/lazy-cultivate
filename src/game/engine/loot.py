...
# ── Global world drops ─────────────────────────────────────────────────────────
# The "can drop anywhere" lane. Entries live in ``src/data/global_drops.json``
# (loaded by GameRegistry as ``registry.global_drops``) and are appended to
# EVERY non-empty loot table by ``GameRegistry.get_loot_table``. Independent-
# mode weights are ABSOLUTE (out of drop.POOL_RANGE = 1,000,000), so the
# chance is uniform per roll regardless of the host table: 100 = 0.01% ≈ one
# per ~10k kills — roughly a week of nonstop AFK farming (1440 turns/day).
# Loot-luck applies. Drop a new entry into the JSON to add a world drop — no
# code change needed (but expect to re-pin the constitution-guard log sha:
# an extra entry shifts the post-win loot RNG stream).


def inject_global_drops(static: list[dict], world_drops: list[dict]) -> list[dict]:
    """Return the world-drop entries to append to a loot table.

    ``world_drops`` is the caller-supplied template — the registry passes its
    JSON-loaded ``global_drops`` so this module stays registry-free. Empty
    static tables stay empty — an unresolvable/placeholder table key must not
    become a stone fountain. Entries are copied so callers can't mutate the
    shared template.
    """
    if not static:
        return []
    return [dict(e) for e in (world_drops or [])]
